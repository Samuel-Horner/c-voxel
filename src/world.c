...
void renderWorld(World *world, ProgramBundle *chunk_program, Chunk **current_chunk_pointer, GLFWwindow *window) {
    ...
    for (int x = -world->lod_render_distance; x < world->lod_render_distance; x++) {
        for (int y = 0; y < world->world_height; y++) {
            for (int z = -world->lod_render_distance; z < world->lod_render_distance; z++) {
                ...
                Chunk *new_chunk = createChunk((ivec3) {world->centre_pos[0] + x, y, world->centre_pos[1] + z}, 0, world->seed, lod);
